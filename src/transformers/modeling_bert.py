...
class BertSelfAttention(nn.Module):
    ...
    def quantize_attention_linear_slinear_clamped(self, att, bits):
        ...
        with torch.no_grad():
            ...
            res = torch.floor((att - min_val) / base + 0.5) * base + min_val
    ...
    def quantize_attention_linear_slog(self, att, bits):
        ...
        with torch.no_grad():
            exp = torch.log2(att)
            clamped_exp = torch.floor(exp / step + 0.5) * step
    ...
    def quantize_attention_linear_slog_midval(self, att, bits):
        ...
        cutpoints = [0.0] + [(i+1)*base for i in range(int(2.0**bits))]
        offset_val = (cutpoints[2]-cutpoints[1])/2.0
        with torch.no_grad():
...
class BertLayer(nn.Module):
    ...
    def quantize_hstates_log(self, inputs, bits):
        ...
        if signs.get_device() != clamped_exp.get_device():
            signs = signs.to(clamped_exp.get_device())
        return torch.pow(2.0, clamped_exp) * signs
